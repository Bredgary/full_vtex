...
def get_params():
    ...
    client = bigquery.Client()
    QUERY = ('SELECT DISTINCT orderId  FROM `shopstar-datalake.staging_zone.order_write`WHERE (orderId NOT IN (SELECT orderId FROM `shopstar-datalake.staging_zone.shopstar_order_connectorResponses`)) limit 50')
    query_job = client.query(QUERY)  
    rows = query_job.result()
    registro = 0
    for row in rows:
