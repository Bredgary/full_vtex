...
'''
QUERY = (
    'select p.idCalculatorConfiguration FROM `shopstar-datalake.landing_zone.shopstar_vtex_all_promotions`,UNNEST(items) AS p WHERE p.idCalculatorConfiguration is not null;')
query_job = client.query(QUERY)  
rows = query_job.result()  

for row in rows:
    productList.append(row.idCalculatorConfiguration)
